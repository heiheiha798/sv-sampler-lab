...
```
执行此命令后，您需要进入各个第三方库的子模块目录（如 `cudd/`、`yosys/`），并根据其自身的说明（通常在其 `README` 文件中或使用 `make` 命令）来编译这些库。这些库的构建方式可能并非都使用 CMake。对于您自己的项目代码，您将在 `build.sh` 中定义编译步骤，其中可以自行选择是否使用 CMake。
